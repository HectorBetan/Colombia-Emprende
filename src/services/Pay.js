...
function Pay() {
  ...
  const handlePagar = async (e) => {
    ...
    let mail = {
      ...
      alt="Logo Colombia Emprende" style="width:300px;" /><h1>Hola <span style="color:#114aa5;">${store.Nombre}</span></h1><h2><span style="color:#114aa5;">${user.displayName}</span> ha pagado la cotización enviada.</h2><div>El usuario ha pagado la cotización y tienes un nuevo pedido creado en tu emprendimiento.<br /> Ve a los pedidos de tu emprendimiento y podrás responder esta solicitud al cliente.<div><br /><div style="font-size:12px; font-weigth:300;">Si sigues el siguiente link debes tener la sesión iniciada, de lo contrario inicia sesión y ve a los pedidos de tu emprendimiento.</div><a href="http://localhost:3000/admin/mi-emprendimiento/pedidos">Ir a los Pedidos de mi Emprendimiento</a></div></div><h3>Gracias por pertenecer a Colombia Emprende</h3></div>`,
