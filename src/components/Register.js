...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    let uid;
    try {
      await signup(user.email, user.password).then((res) => {
        uid = res.user.uid;
      });
    } catch (error) {
  ...
  const ProfilePhoto = () => {
    if (img) {
      ...
      return <PhotoView img={imgUrl} s="60px" />;
    } else {
      return <UserLogo w="60" h="60" />;
    }
  };
  if (msg) {
    setTimeout(() => {
    ...
    <div>
      ...
      <form onSubmit={handleRegister}>
        ...
        <div className="justify-content-center m-lg-3 m-xl-3 m-xxl-3 m-md-3">
          <label className="d-block m-1 mt-2 me-2 text-center">
          ...
          <div className="d-column d-lg-flex d-xl-flex d-xxl-flex flex-row text-center">
            ...
            <input
              type="file"
              className="w-100 ms-lg-3 ms-xl-3 ms-xxl-3 mt-3 subirFoto"
              onChange={changeImg}
              accept="image/*"
            ></input>
          </div>
        ...
        <span className="d-flex flex-row justify-content-center mb-1 mt-3 policy">
          <input
            ...
            required
          />
          <span className="politicas-registro">
            Acepto las&nbsp;
            <PolicyModal />
          </span>
        </span>
