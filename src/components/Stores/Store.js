...
function Store(data) {
  ...
  const emprendimiento = lista.find(
          ...
          {emprendimiento.value.products.map((producto, i) => {
            if (!producto.Delete) {
              ...
              return (
                <div
                  key={producto._id}
                  className="card p-2 p-sm-3 p-md-3 p-lg-3 p-xl-3 p-xxl-3 mb-3"
                >
                  <div className="d-flex flex-row justify-content-between">
                    <div className="d-flex flex-row  justify-content-between">
                      ...
                      <div className="d-flex flex-column ms-2 me-3 ms-md-3 me-md-2 ps-md-2 pe-md-2 ms-lg-3 me-lg-3 ps-lg-3 pe-lg-3 ms-xl-3 me-xl-3 ps-xl-3 pe-xl-3 ms-xxl-3 me-xxl-3 ps-xxl-3 pe-xxl-3  justify-content-center producto-data">
                        ...
                        {producto.Descripcion && w > 600 && (
                          <div className="ps-2 pe-2 desc">
                            Descripción: <span>{desc}</span>
                            {verMas && (
                              <span className="ver-mas-descripcion">
                                <PopOver
                                  data={{ desc: producto.Descripcion, i: i }}
                                />
                              </span>
                            )}
                          </div>
                    ...
                    {w >= 750 && (
                      <div className="d-flex flex-column justify-content-center">
                        ...
                        <button
                          className="btn btn-primary boton-agregar-carrito"
                          onClick={(e) => {
                            e.preventDefault();
                            agregar(producto);
                          }}
                  ...
                  {w < 750 && (
                    <div className="d-flex flex-row justify-content-center mt-3 stores-btn-a">
                      ...
                      <button
                        className="btn btn-primary boton-agregar-carrito-cel"
                        onClick={(e) => {
                          e.preventDefault();
                          agregar(producto);
                        }}
                  ...
                  <div
                    className="m-2 d-none text-center"
                    id={`${producto._id}success`}
                  >
                    <Alert />
                  </div>
                </div>
  ...
  const PhotosViewCel = () => {
    ...
    if (fotos) {
      ...
      return (
        <img className="d-block imagen-btn-cel" src={imgStore} alt="fotos" />
      );
    }
  };
  const PhotosView = () => {
            ...
            <div className="d-flex flex-row m-2">
              <div className="d-flex flex-column justify-content-center mt-1">
                {fotos.map((img, i) => {
                  return (
                    <button
                      className="botn-imagen btn btn-ligth p-0"
                      key={i}
                      onClick={(e) => {
                        e.preventDefault();
                        setSelected(i);
  ...
  const CalificacionView = () => {
    ...
    let nums = [1, 2, 3, 4, 5];
    return (
      <div className="d-flex flex-row sstars">
        {nums.map((n) => {
          return (
            <div key={n}>
              {n <= total && (
                <i className="fa-solid fa-star star-1 estrella" id="star-1" />
              )}
              {n > total && (
                <i
                  className="fa-regular fa-star star-1 estrella-vacia"
                  id="star-1"
                />
              )}
            </div>
        ...
        <div className="card pb-3">
          <div className="d-flex flex-row justify-content-start">
            ...
            <div className="card-body  d-flex flex-column justify-content-center caja-datos">
              ...
              {emprendimiento.value.store.Calificacion && (
                <div>{w >= 600 && <CalificacionView />}</div>
              )}
              {w >= 600 && (
              ...
              {w < 600 && (
                <div className="d-flex flex-row mb-2">
                  ...
                  <div className="d-flex flex-column">
                    <h5 className="card-title store-title store-title-1">
                      {emprendimiento.value.store.Nombre}
                    </h5>
                    <h4 className="card-text store-categoria">
              ...
              {emprendimiento.value.store.Calificacion && w < 600 && (
                <div className="text-center d-flex flex-row">
                  <span className="me-1">Calificación:</span>
                  <CalificacionView />
                </div>
              )}
              <h3 className="card-text store-ciudad">
                <span>Ciudad: </span>
                <span className="stores-cel">
                  {emprendimiento.value.store.Ciudad}
                </span>
              </h3>
              <h3 className="card-text store-celular">
                {w >= 600 && <span>Celular: </span>}
                {w < 600 && <span>Celular: </span>}
                <span className="stores-cel">
                  {emprendimiento.value.store.Celular}
                </span>
              </h3>
              ...
              {emprendimiento.value.store.Direccion && (
                <h4 className="card-text store-categoria">
                  {w >= 800 && <span>Dirección: </span>}
                  <span className="stores-cel">
                    {emprendimiento.value.store.Direccion}
                  </span>
                </h4>
                ...
                <span>
                  {w > 300 && (
                    <h4 className="card-text store-email">
                      ...
                      <span className="stores-cel">
                        {emprendimiento.value.store.Email}
                      </span>
                    </h4>
                  )}
                </span>
          ...
          {(emprendimiento.value.store.Facebook ||
            emprendimiento.value.store.Instagram ||
            emprendimiento.value.Web) && <hr />}
          <div className="d-flex flex-row justify-content-center ">
            {emprendimiento.value.store.Facebook && (
              <span>
                <a
                  role="button"
                  className="btn botonredes ms-4 me-4"
                  href={face}
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            {emprendimiento.value.store.Web && (
              <span>
                <a
                  role="button"
                  href={web}
  ...
  if (cargando) {
    ...
    return (
      <div className="text-center">
        ...
        <div
          className="accordion ms-4 me-4 ms-md-4 me-md-4 ms-lg-5 me-lg-5 ms-xl-5 me-xl-5 ms-xxl-5 me-xxl-5 mb-4"
          id="accordionStore"
        >
          <div className="accordion-item">
            <h2 className="accordion-header" id="accordionStore-headingOne">
              <button
                className="accordion-button  acordeon-tienda"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#accordionStore-collapseOne"
                aria-expanded="true"
                aria-controls="accordionStore-collapseOne"
              >
                Información de {emprendimiento.value.store.Nombre}
              </button>
            </h2>
            <div
              id="accordionStore-collapseOne"
              className="accordion-collapse collapse show"
              aria-labelledby="accordionStore-headingOne"
            >
          ...
          {emprendimiento.value.products.length > 0 && (
            <div className="accordion-item">
              <h2 className="accordion-header" id="accordionStore-headingTwo">
                <button
                  className="accordion-button collapsed acordeon-tienda"
                  type="button"
                  data-bs-toggle="collapse"
                  data-bs-target="#accordionStore-collapseTwo"
                  aria-expanded="false"
                  aria-controls="accordionStore-collapseTwo"
                >
                  Productos de {emprendimiento.value.store.Nombre}
                </button>
              </h2>
              <div
                id="accordionStore-collapseTwo"
                className="accordion-collapse collapse show"
                aria-labelledby="accordionStore-headingTwo"
              >
                <div className="accordion-body  text-start pe-4 ps-4">
                  <ExistsProducts />
                </div>
