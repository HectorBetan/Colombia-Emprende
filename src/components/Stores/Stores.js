    ...
    if (newList){
      return (
        <div className="">
          ...
          <input className="form-input busqueda" type="number" min="1" max="10" defaultValue="5"onChange={(e)=>{e.preventDefault();setActive(1);if(!e.target.value || e.target.value <1){setResultados(0)} if(e.target.value >10){setResultados(9)}else{setResultados(parseInt(e.target.value)-1)}; console.log(resultados)}}></input>
